...
1. **看摘要**：顶部黄色块显示 3 条 P0/P1 客户待回复、2 个研发任务有上线承诺/联调依赖风险、1 个功能待客户确认、2 条 VOC 疑似缺关联。
2. **看待办**：3 张卡片（沃尔玛 P0 / 奔驰 P1 / 地铁 P1），每张带建议回复要点 + 依据 chip。
3. **看风险**：4 条，覆盖三种类型 —— 已上线未确认 (DEV-104)、上线承诺 (DEV-101 / DEV-106)、联调依赖 (DEV-106)。
4. **看待人工确认**：3 条，包括 VOC-003↔DEV-103、VOC-006↔DEV-105，以及 DEV-104 待客户确认。
...
页面底部有一个 **原始数据（mock）** 折叠区，可以直接查看 VOC / DEV / 群聊三类原始数据；上面的摘要、待办、风险和待确认是整理后的结果。

## 交付说明

**我如何理解这个需求**：PM 每天要把 VOC、研发任务、群聊三类信息拼在一起，判断今天该回复谁、哪些研发事项有风险、哪些关联需要补确认。这个 PoC 的目标不是替 PM 自动做决定，而是先把散乱信息整理成可解释的今日清单，让 PM 在关键节点上确认。

**SPEC 是否发生过调整**：发生过。最初设想包含 LLM、文件上传、后端存储等能力，后来收敛为 SPEC v1：纯规则、本地 mock 数据、无后端。原因是 4 小时交付更需要证明主链路是否成立，而不是堆完整产品形态。

**做了哪些取舍**：

- 保留：依据追溯、HITL 人工确认、导出 markdown、自动验收脚本。
- 砍掉：LLM、文件上传、数据库、真实 IM 接入、登录权限。
- 延后：更完整的话术生成、历史记录、多用户协作、真实数据接入。

**用了哪些 AI 编程工具**：主要使用 Codex / Claude Code 类 AI 编程助手完成 SPEC 草稿、代码实现、self-review、README / 复盘 / AI_LOG 整理；人工负责范围裁剪、业务口径修正和最终验收。

**AI 协作过程记录在哪里**：[AI_LOG.md](./AI_LOG.md)。

**哪些地方是 mock**：

- `src/data/voc.js`：VOC 表 mock 数据
- `src/data/dev.js`：研发任务 mock 数据
- `src/data/chat.js`：群聊 mock 数据
- 前端 state：人工确认、已回复、重置等状态只存在浏览器内存，刷新会恢复初始状态

**如果继续做成真实产品，下一步怎么做**：

1. 接入真实数据：支持上传 CSV / 粘贴表格 / 接 IM 导出记录。
2. 引入持久化：先用 localStorage，再扩展到后端数据库和多人协作。
3. 替换群聊解析：把 `parseChat` 从规则升级为 LLM 结构化抽取，并要求输出依据和置信度。
4. 加强 HITL：低置信度判断进入待确认，高风险动作必须人工拍板。
5. 补齐产品闭环：历史日报、导出模板、团队字段配置、权限和审计。
